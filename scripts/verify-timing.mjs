...
const year=new Date().getUTCFullYear(),args={timezone:'UTC',day_start_hour:0,start_date:`${year}-01-01`,end_date:`${year}-12-31`,max_review_seconds:60,max_reveal_seconds:30};
...
const revealMs=grades.map(e=>e.revealTime).filter(v=>typeof v==='number'&&Number.isFinite(v)&&v>=0&&v<=Number.MAX_SAFE_INTEGER).sort((a,b)=>a-b);
const revealSeconds=revealMs.map(v=>v/1000);
assert.equal(summary.timing.reveal.unfiltered.samples,revealSeconds.length);
assert.equal(summary.timing.reveal.unfiltered.recorded_seconds,sum(revealSeconds));
assert.equal(summary.timing.reveal.filtered.recorded_seconds,sum(revealSeconds.filter(v=>v<=30)));
assert.equal(summary.timing.reveal.unfiltered.median_seconds,revealSeconds.length?(revealSeconds[Math.floor((revealSeconds.length-1)/2)]+revealSeconds[Math.ceil((revealSeconds.length-1)/2)])/2:null);
...
console.log(JSON.stringify({verified:true,graded_events_checked:grades.length,timed_events_checked:ms.length,raw_and_filtered_totals_match:true,median_matches:true,five_views_checked:true,separate_reveal_statistics_verified:true}));
