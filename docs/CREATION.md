...
- `front` and basic `back`: nonblank literal strings. Markdown, arrows, `>>` and similar text are never parsed as formatting or extra flashcards. Use `front_images`, basic `back_images`, or multiline `back.items[].images` to append hosted or reused images; see the [image guide](IMAGES.md). Other rich formatting and nested-answer creation remain unsupported.
