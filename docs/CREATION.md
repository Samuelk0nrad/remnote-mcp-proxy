...
- `front` and basic `back`: nonblank literal strings or explicit formatted span objects. Multiline item `text` and context notes accept the same structure; see [formatting](FORMATTING.md). Markdown, arrows, `>>` and similar text are never parsed as formatting or extra flashcards. Use `front_images`, basic `back_images`, or multiline `back.items[].images` to append hosted or reused images; see the [image guide](IMAGES.md). Bold, italic and underline use explicit text spans. Other rich formatting and nested-answer creation remain unsupported.
