...
test('lists official and Edit Later tools and reads the real queue', async (t) => {
  ...
  assert.deepEqual(
    ...
    ['search_docs', 'get_edit_later_queue', 'update_rem', 'resolve_edit_later_item', 'delete_rem'],
  );
...
test('deletes one non-document Rem and refuses document Rems', async (t) => {
  const token = 'test-token';
  const databasePath = createFixtureDatabase();
  const calls = [];
  const proxy = http.createServer(
    createMcpHandler({
      ...
      runtimeMcpRunner: async (name, args) => {
        ...
        if (args.operation === 'state') return { isDocument: args.remId === 'documentRem1' };
        return { applied: true };
      },
      logger: { error() {} },
    }),
  ...
  const deleted = await rpc(url, token, {
    jsonrpc: '2.0',
    id: 1,
    method: 'tools/call',
    params: { name: 'delete_rem', arguments: { rem_id: 'childRem1' } },
  });
  assert.equal(deleted.body.result.structuredContent.ok, true);
  assert.equal(deleted.body.result.structuredContent.deleted, true);
  assert.deepEqual(calls.slice(0, 2), [
    { name: 'remnote_rem', args: { operation: 'state', remId: 'childRem1' } },
    { name: 'remnote_rem', args: { operation: 'remove', remId: 'childRem1' } },
  ]);
  ...
  const refused = await rpc(url, token, {
    ...
    id: 2,
    method: 'tools/call',
    params: { name: 'delete_rem', arguments: { rem_id: 'documentRem1' } },
  });
  assert.equal(refused.body.error.code, -32000);
  assert.match(refused.body.error.data.message, /refuses to delete document Rems/);
  assert.deepEqual(calls.slice(2), [
    { name: 'remnote_rem', args: { operation: 'state', remId: 'documentRem1' } },
  ]);

  const invalid = await rpc(url, token, {
    jsonrpc: '2.0',
    id: 3,
    method: 'tools/call',
    params: { name: 'delete_rem', arguments: { rem_id: 'x', extra: true } },
  });
